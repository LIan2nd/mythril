# d6a0a6f3-70d7-4271-b16e-af4af7624a0a implementation handoff

This archive is the source of truth for turning the design into production code. Start from `mythril-dashboard.html`, then preserve the visual system, responsive behavior, and interactions found in the exported files.
...
## Source map
- Primary entry: `mythril-dashboard.html`
...
- A single self-contained `mythril-dashboard.html` is acceptable only when the export truly contains one user-facing screen and its CSS/JS are structured enough to extract tokens, components, states, and behavior.
...
1. Open `mythril-dashboard.html` and `DESIGN-MANIFEST.json`; identify every screen file, launcher/overview file, app module, and interaction before coding.
...
1. Inspect `mythril-dashboard.html` and `DESIGN-MANIFEST.json` first and identify reusable components before coding.
